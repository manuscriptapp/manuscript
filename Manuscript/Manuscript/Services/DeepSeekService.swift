...
/// Service for making requests to the DeepSeek API.
/// DeepSeek uses an OpenAI-compatible API format.
actor DeepSeekService: AnyModelProvider {
    ...
    private func getAPIKey() throws -> String {
        guard let apiKey = AISettingsManager.shared.getDeepSeekKey(), !apiKey.isEmpty else {
            throw AnyModelError.noAPIKey(.deepSeek)
        }
        return apiKey
    }
    ...
    private func performRequest(_ request: ChatCompletionRequest, apiKey: String) async throws -> String {
        ...
        do {
            ...
            case 200...299:
                break
            case 401:
                throw AnyModelError.invalidAPIKey(.deepSeek)
            case 429:
